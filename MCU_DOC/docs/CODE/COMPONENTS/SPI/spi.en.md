...
!!! info
    `spi.h` & `spi.c` provide functions to configure the SPI buses. For ESP32S3, there are only SPI buses for user uses. One SPI bus can be shared by differentt devices by using chip selection mechanism. This driver shows how to setup and config the SPI bus. As for how to add devices onto the bus and how to use chip selection, please refer to respective drivers.
