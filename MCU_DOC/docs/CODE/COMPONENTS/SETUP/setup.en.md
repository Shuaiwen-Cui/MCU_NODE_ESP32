...
!!! info "SETUP.H & SETUP.C"
    These files are used to configure all the components under the `components` directory. The `setup.h` file is used to define the configuration of the components.

!!! tip "how do they work?"
    The `setup.h` file is used to define the configuration of the components. The `setup.c` file is used to implement the configuration of the components. `setup.h` is contained in each head file of a component, and then, the head file of each component is included in the `main.h` file. Finally, the `main.h` file is included in the `main.c` file.
