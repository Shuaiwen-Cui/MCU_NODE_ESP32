# `setup.h` 和 `setup.c` 文件

!!! info "SETUP.H 和 SETUP.C"
    这些文件用于配置 `components` 目录下的所有组件。`setup.h` 文件用于定义组件的配置。

!!! tip "它们是如何工作的?"
    `setup.h` 文件用于定义组件的配置。`setup.c` 文件用于实现组件的配置。`setup.h` 包含在每个组件的头文件中，然后，每个组件的头文件包含在 `main.h` 文件中。最后，`main.h` 文件包含在 `main.c` 文件中。
